<template>
  <div>
    <div>
      <p class="text-white cursor-pointer" @click="isMenuShown = !isMenuShown">
        {{ state.name }}
      </p>
      <div v-if="isMenuShown" class="w-40 absolute text-header rounded-md px-2 py-0.5 bg-white z-50 right-5 top-12">
        <div class="py-1 text-right w-full">
          <b-button class="w-full" :class="{ 'cursor-not-allowed': isDisabled }" :disabled="isDisabled" variant="secondary" size="sm">
            Выйти
          </b-button>
        </div>
      </div>
